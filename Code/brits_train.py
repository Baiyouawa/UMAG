...
def diffusion_train(configs):
    ...
    model = brits.BRITS(
        ...
        rnn_hid_size=getattr(configs, "rnn_hid_size", 100),
        impute_weight=getattr(configs, "impute_weight", 3.0),
        consistency_weight=getattr(configs, "consistency_weight", 0.1),
    ).to(configs.device)
    ...
    for epoch in range(configs.epoch_diff):
        model.train()
        train_loss = []
        for observed_data, _, observed_mask, _, gt_mask in tqdm(
            ...
            eval_mask = (gt_mask - observed_mask).clamp(min=0.0)

            optimizer.zero_grad()
            imputed, loss = model(observed_data, observed_mask, eval_mask)
            loss.backward()
            optimizer.step()
...
def diffusion_test(configs, model):
    ...
    with torch.no_grad():
        for observed_data, _, observed_mask, _, gt_mask in tqdm(test_loader, desc="Test", leave=False):
            observed_data = observed_data.to(configs.device)
            observed_mask = observed_mask.to(configs.device)
            gt_mask = gt_mask.to(configs.device)

            eval_mask = (gt_mask - observed_mask).clamp(min=0.0)
            imputed, _ = model(observed_data, observed_mask, eval_mask)

            imputed_data = observed_mask * observed_data + (1 - observed_mask) * imputed
            evalmask = eval_mask

            target_2d.append(observed_data.detach().cpu())
            forecast_2d.append(imputed_data.detach().cpu())
            eval_p_2d.append(evalmask.detach().cpu())

            B, L, K = imputed_data.shape
            temp = imputed_data.reshape(B * L, K).detach().cpu().numpy()
    ...
    out_path = results_root / f"BRITS_{configs.dataset}_p{int(configs.missing_rate*100)}_seed{configs.seed}.csv"
    np.savetxt(out_path, generate_data2d, delimiter=",")
    print(f"BRITS imputation results saved to {out_path}")
