...
from models.grin import GRINWrapper


def diffusion_train(configs):
    train_loader, _ = A_dataset.get_dataset(configs)
    model = GRINWrapper(configs).to(configs.device)
    ...
    for epoch in range(configs.epoch_diff):
        ...
        for observed_data, _, observed_mask, _, gt_mask in tqdm(
            ...
            optimizer.zero_grad()
            imputed = model(observed_data, observed_mask)
            loss = torch.mean(((imputed - observed_data) * eval_mask) ** 2)
            loss.backward()
            optimizer.step()
...
def diffusion_test(configs, model):
    ...
    with torch.no_grad():
        for observed_data, _, observed_mask, _, gt_mask in tqdm(test_loader, desc="Test", leave=False):
            ...
            target_2d.append(observed_data.cpu())
    ...
    out_path = results_root / f"GRIN_{configs.dataset}_p{int(configs.missing_rate*100)}_seed{configs.seed}.csv"
    np.savetxt(out_path, generate_data2d, delimiter=",")
    print(f"GRIN imputation results saved to {out_path}")
