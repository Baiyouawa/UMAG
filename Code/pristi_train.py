...
from models.pristi import PriSTIWrapper


def diffusion_train(configs):
    train_loader, _ = A_dataset.get_dataset(configs)
    model = PriSTIWrapper(configs).to(configs.device)
    ...
    for epoch in range(configs.epoch_diff):
        ...
        for observed_data, _, observed_mask, observed_tp, gt_mask in tqdm(
            ...
            optimizer.zero_grad()
            loss = model.compute_loss(observed_data, observed_mask, observed_tp, gt_mask)
            loss.backward()
            optimizer.step()
        ...
        if epoch % 50 == 0 or epoch == configs.epoch_diff - 1:
            print(f"Epoch: {epoch+1}. Train_loss: {np.average(train_loss) if train_loss else 0:.6f}")
    return model
...
def diffusion_test(configs, model):
    ...
    with torch.no_grad():
        for observed_data, _, observed_mask, observed_tp, gt_mask in tqdm(test_loader, desc="Test", leave=False):
            ...
            imputed = model.impute(observed_data, observed_mask, observed_tp, gt_mask, n_samples=50)
    ...
    out_path = results_root / f"PriSTI_{configs.dataset}_p{int(configs.missing_rate*100)}_seed{configs.seed}.csv"
    np.savetxt(out_path, generate_data2d, delimiter=",")
    print(f"PriSTI imputation results saved to {out_path}")
