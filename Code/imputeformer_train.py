...
from models.imputeformer import ImputeFormerWrapper


def diffusion_train(configs):
    train_loader, _ = A_dataset.get_dataset(configs)
    model = ImputeFormerWrapper(configs).to(configs.device)
    ...
    for epoch in range(configs.epoch_diff):
        ...
        for observed_data, _, observed_mask, observed_tp, gt_mask in tqdm(
            ...
            optimizer.zero_grad()
            imputed = model(observed_data, observed_mask, observed_tp)
...
def diffusion_test(configs, model):
    ...
    out_path = results_root / f"ImputeFormer_{configs.dataset}_p{int(configs.missing_rate*100)}_seed{configs.seed}.csv"
    np.savetxt(out_path, generate_data2d, delimiter=",")
    print(f"ImputeFormer imputation results saved to {out_path}")
