...
from models.spin import SPINWrapper


def diffusion_train(configs):
    train_loader, _ = A_dataset.get_dataset(configs)
    model = SPINWrapper(configs).to(configs.device)
...
def diffusion_test(configs, model):
    ...
    out_path = results_root / f"SPIN_{configs.dataset}_p{int(configs.missing_rate*100)}_seed{configs.seed}.csv"
    np.savetxt(out_path, generate_data2d, delimiter=",")
    print(f"SPIN imputation results saved to {out_path}")
